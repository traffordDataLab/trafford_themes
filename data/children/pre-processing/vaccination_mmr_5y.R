...
df_raw <- read_csv(paste0("https://webservices.esd.org.uk/data.csv?metricType=12109&area=",paste(c("E92000001",authorities$area_code, "E08000009"), collapse = ','),"&period=latest:8&columnGrouping=period&rowGrouping=area&ApplicationKey=",lginform_key))
...
df <- df_raw %>%
  ...
  mutate(measure = "Percentage",
         unit = "Persons",
         indicator = "Population vaccination coverage - MMR for two doses (5 years old)",
...
write_csv(df, "../vaccination_mmr_5y.csv")
