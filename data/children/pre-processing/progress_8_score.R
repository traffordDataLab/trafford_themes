...
df_raw <- read_csv(paste0("https://webservices.esd.org.uk/data.csv?metricType=6016&area=",paste(c("E92000001",authorities$area_code, "E08000009"), collapse = ','),"&period=latest:8&columnGrouping=period&rowGrouping=area&ApplicationKey=",lginform_key))
...
df <- df_raw %>%
  ...
  mutate(measure = "Percentage",
         unit = "Persons",
         indicator = "Average Progress 8 score",
...
write_csv(df, "../progress_8_score.csv")
