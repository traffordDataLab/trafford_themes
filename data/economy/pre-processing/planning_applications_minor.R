...
df_raw <- read_csv(paste0("https://webservices.esd.org.uk/data.csv?metricType=17487&area=",paste(c("E92000001",authorities$area_code, "E08000009"), collapse = ','),"&period=latest:13&columnGrouping=period&rowGrouping=area&ApplicationKey=",lginform_key))
...
df <- df_raw %>%
  ...
  mutate(measure = "Percentage",
         unit = "Applications",
         indicator = "Percentage of all minor development planning applications decided in time",
...
write_csv(df, "../planning_applications_minor.csv")
