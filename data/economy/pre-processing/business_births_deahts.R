...
GET(url = "https://www.ons.gov.uk/file?uri=/businessindustryandtrade/business/activitysizeandlocation/datasets/businessdemographyquarterlyexperimentalstatisticslowlevelgeographicbreakdownuk/quarter2apriltojune2024/finalq22024lowlevelgeobreakdown.xlsx",
    write_disk(tmp))
...
df <- set_names(sheets[3:12]) %>%
  ...
  pivot_longer(`Q1 2017`:`Q2 2024`, names_to = "period", values_to = "Count") %>%
  filter(!is.na(Count)) %>%
