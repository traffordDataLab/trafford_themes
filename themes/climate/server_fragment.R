...
output$licensed_vehicles_plot <- renderGirafe({
  gg <- ggplot(df_licensed_vehicles,
               ...
               labs(title = "All Licensed vehicles",
                    ...
                    y = "Count",
                    fill = NULL,
                    alt = "Line chart showing the number of vehicles registered to Trafford addresses has been consistently lower compared to the average of similar authorities over the 3 year period from December 2022 to September 2025. The numbers in Trafford for each calendar quarter are consistently around 130,000. The average for similar authorities has been much higher compared to Trafford, with around 206,000 - 217,000 licensed vehicles each calendar quarter. The latest data shows 132,373 vehicles registered within Trafford compared with an average of 216,328 vehicles in similar authorities.") +
               theme_x()
...
output$licensed_ulev_plot <- renderGirafe({
  gg <- ggplot(df_licensed_ulev,
    ...
    labs(title = "Proportion of Ultra Low Emission Vehicles",
         ...
         y = "Percentage",
         fill = NULL,
         alt = "Line chart showing the proportion of licensed vehicles registered to Trafford addresses which are ultra low emission vehicles has been consistently lower, and increasing at a slower rate, compared to the average for similar authorities and England across the 3 year period from December 2022 to September 2025. The proportion in Trafford has risen from 2.18% to 5.2% in period shown, compared with the average for similar authorities from 6.16% to 13.51% and the national average from 3.02% to 6.77%.") +
    theme_x()
...
output$ev_charging_points_plot <- renderGirafe({
  gg <- ggplot(df_ev_charging_points_rate,
    ...
    labs(title = "Publicly available electric vehicle charging devices",
         ...
         y = "Devices (per 100K)",
         fill = NULL,
         alt = "Line chart showing publicly available charging devices per 100,000 people in Trafford compared to the average of similar authorities and England between April 2023 and January 2026. Up to October 2023 Trafford's rate was consistently much lower than its comparitors, however since a significant increase in January 2024 it's been more in line with the average of similar authorities, but still below the national rate. The latest data shows Trafford having 115.3 devices per 100,000 population, above the average for similar authorities of 98.2 devices but below the national average of 129.3.") +
    theme_x()
  ...
  if (input$household_waste_recycling_selection == "% Trend") {
    ...
    gg <- ggplot(df_household_waste_recycling_percentage,
      ...
      labs(title = "Household waste collected and sent for recycling",
           ...
           y = "Percentage",
           fill = NULL,
           alt = "Line chart showing the percentage of household waste collected and sent for recycling in Trafford compared with similar authorities and England between 2013/14 and 2024/25. Figures in Trafford have been higher than the average of similar authorities and England. Whilst the trend for its comparitors has remained broadly consistent, Trafford's has been more variable. The increase between 2013/14 (57.5%) and 2014/15 (61.9%) preceeded a downward trend to 53.3% in 2020/21. Since then Trafford's percentage rose above 58% for the next 3 periods. The latest figure for 2024/25 shows 57.5% in Trafford compared to 45.3% for the average of similar authorities and 42% for the England average.") +
      theme_x()
    ...
    gg <- ggplot(df_household_waste_recycling_tonnes,
      ...
      labs(title = "Household waste collected and sent for recycling",
           ...
           y = "Tonnes",
           fill = NULL,
           alt = "Line chart showing between 2014/15 and 2016/17 Trafford collected and sent for recycling around 50,000 tonnes of household waste, approximately 6,000 more than the average for similar authorities. However, from 2018/19 onwards the amounts have been broadly the same. The latest data for 2024/25 shows Trafford collected and sent 43,777 tonnes of household waste for recycling, compared with 45,774 tonnes for the average of similar local authorities.") +
      theme_x()
  ...
  if (input$household_waste_not_recycled_selection == "% Trend") {
    ...
    gg <- ggplot(df_household_waste_not_recycled_percentage,
      ...
      labs(title = "Household waste collected not sent for recycling",
           ...
           y = "Percentage",
           fill = NULL,
           alt = "Line chart showing that Trafford has had a lower percentage of household waste collected but not sent for recycling than the average of similar authorities and England between 2013/14 and 2024/25, with the gap narrowing (and therefore the percentage rising) between 2016/17 and 2020/21 from 38.7% to 46.7%. The percentage then dropped to 41.2% before slightly rising for last 4 periods. The trend shows the national average being the highest throughout the period, with the average for similar authorities to Trafford just below tracking the same trend. The latest data for 2024/25 shows 42.5% for Trafford compared to 54.7% for the average of similar authorities and 58% for the England average.") +
      theme_x()
    ...
    gg <- ggplot(df_household_waste_not_recycled_tonnes,
      ...
      labs(title = "Household waste collected not sent for recycling",
           ...
           y = "Tonnes",
           fill = NULL,
           alt = "Line chart showing Trafford has had a lower tonnage of household waste that was collected but not sent for recycling compared to the average of similar authorities between 2014/15 and 2024/25. Following a sharp increase in 2020/21 for both Trafford and its comparator, Trafford's tonnage by 2022/23 (25,022) had reduced by a greater amount than the average of similar authorities (39,516.8). The latest data for 2024/25 shows 26,136 tonnes for Trafford compared with 39,383.8 tonnes for the average of similar authorities.") +
      theme_x()
...
df_epc <- bind_rows(calc10YearProportionEPC("2013-06-30", "2023-03-31"),
                    ...
                    calc10YearProportionEPC("2015-09-30", "2025-06-30"),
                    calc10YearProportionEPC("2015-12-31", "2025-09-30"),
                    calc10YearProportionEPC("2016-03-31", "2025-12-31"))
...
output$domestic_epc_plot <- renderGirafe({
  gg <- ggplot(df_epc,
    ...
    labs(title = "Domestic EPC rated A, B or C over 10 years",
         ...
         y = "Percentage",
         fill = NULL,
         alt = "Line chart showing that over the 10 year periods starting Jun 2013 to Mar 2023 through to Mar 2016 to Dec 2025 Trafford has consistently lower percentages of domestic properties with Energy Performance Certificates (EPC) rated A, B or C than the average of similar authorities (approximately 11 percentage points fewer) or England (approximately 7 percentage points fewer). Data for Trafford goes from 35.3% to 46.1% across the period compared with 47.0% to 56.9% for the average of similar authorities and 42.6% to 51.8% for the England average.") +
    theme_x()
...
output$borough_co2_emissions_plot <- renderGirafe({
  gg <- ggplot(df_borough_co2_emissions,
    ...
    labs(title = expression(paste("Territorial Carbon Dioxide (", CO[2], ") emission estimates")),
         ...
         fill = NULL,
         alt = "Line chart showing that territorial carbon dioxide (CO2) emissions in Trafford were higher than the average for similar authorities between 2012 and 2023. Although the amount has been decreasing since 2012, the trend has been very similar to the average for similar authorities. In 2020, emissions for both Trafford and the average of similar local authorities dropped corresponding with the start of the coronavirus pandemic, and Trafford's emissions were the closest to those of its comparitor in the period shown. Since then the gap has widened, however the trend has remained consistent. The latest data for 2023 shows an estimated 1,301.2 kilotonnes of CO2 emitted within the borough of Trafford compared to the similar authorities average of 1,142.8 kilotonnes.") +
    theme_x()
...
output$no2_concentration_plot <- renderGirafe({
  gg <- 
    ...
    labs(title = expression(paste("Annual mean Nitrogen Dioxide (", NO[2], ") concentration")),
         ...
         fill = NULL,
         colour = "Location: ",
         alt = "Line chart showing the annual mean of N.O.2 readings (microgrammes per cubic metre) taken between the 12 months ending June 2023 and the 12 months ending March 2026 at Trafford A56, Trafford Moss Park and Trafford Wellacre Academy monitoring stations. Readings from A56 are the highest across the period but on a downward trend and the latest reading of 16.9 is the lowest value in the period for this station. Readings at Moss Park between the 12 months ending 2023-09-30 and 2024-12-31 were higher than Wellacre but declining and are now lower, with the latest average of 8.7. Readings for Wellacre Academy have generally been between 10 and 11, except for 3 of the last 4 periods. The lowest average reading was 7.7 for the 12 months ending 2025-09-30 and the latest average reading is 9.5.") +
    theme_x() +
    theme(
      legend.position = "top",
...
output$pm10_concentration_plot <- renderGirafe({
  gg <- 
    ...
    labs(title = expression(paste("Annual mean Particulate Matter (", PM[10], ") concentration")),
         ...
         fill = NULL,
         colour = "Location: ",
         alt = "Line chart showing the annual mean of PM10 readings (microgrammes per cubic metre) taken between the 12 months ending June 2023 and the 12 months ending March 2026 at Trafford A56 and Trafford Moss Park monitoring stations. Readings at A56 have been higher for 10 of the 12 periods and show a falling - rising - falling trend with the lowest average reading of 12.1 in the 12 months ending 2024-09-30, the highest of 15.6 in the 12 months ending 2025-06-30 and the latest of 13.4. The Moss Park trend is similar but with lower variation, with the highest average reading of 13.8 at the start of the period, a low of 10.2 in the 12 months ending 2024-06-30 and the latest of 11.3.") +
    theme_x() +
    theme(
      legend.position = "top",
      legend.title = element_text(size = 9),
...
output$industry_emissions_plot <- renderGirafe({
  gg <- ggplot(df_industry_emissions,
    ...
    labs(title = "Greenhouse gas industrial emissions",
         ...
         y = "Kilotonnes (ktCO2e)",
         fill = NULL,
         alt = "Line chart showing territorial greenhouse gas industrial emissions estimates in Trafford compared to the average of similar authorities between 2012 and 2023. 2012: Trafford 684.5, Similar Authorities average 589.8 ktCO2e. 2023: Trafford 517.3, Similar Authorities average 254.1 ktCO2e. Trafford is above its comparator for the time period shown and despite a sharp decrease in 2020, levels increased again the following year unlike its comparator. Although both trend lines are decreasing, the rate of decline for the average for similar local authorities is faster than Trafford."
    ) +
    theme_x()
